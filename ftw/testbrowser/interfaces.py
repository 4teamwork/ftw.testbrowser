...
class IRESTAPIClient(Interface):

    def open(url_or_object, data=None, endpoint=None):
        """Open the REST API Client and go to an URL or visit the default
        endpoint of an object.
        ...
        ``endpoint`` - a REST API endpoint name (string or unicode), which is
        appended to the URL. This is especially useful combined with passing in
        objects.
        """
